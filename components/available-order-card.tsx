...
import { Clock, FileText, Calendar, BookOpen, GraduationCap, Sparkles } from "lucide-react"
...
export function AvailableOrderCard({ order }: AvailableOrderCardProps) {
  ...
  const writerEarning = order.customerIsReturning ? order.totalAmount * 0.45 : order.totalAmount * 0.4
    ...
    <Card className="bg-slate-900 border-slate-800 hover:border-primary/50 transition-all duration-300">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between mb-2">
          ...
          <Badge className="bg-blue-500/10 text-blue-500 border-blue-500/20">Available</Badge>
        </div>
        <CardTitle className="text-lg text-slate-100 leading-tight">{order.title}</CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-slate-200 font-medium">{order.educationLevel}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 text-sm pt-2 border-t border-slate-800">
          <div>
            ...
            <p className="text-slate-200 font-semibold">{order.pagesOrSlides}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-1">Words</p>
            <p className="text-slate-200 font-semibold">{order.words.toLocaleString()}</p>
          </div>
          <div>
            <p className="text-slate-500 text-xs mb-1">Sources</p>
            <p className="text-slate-200 font-semibold">{order.sourcesRequired}</p>
          </div>
        </div>
        ...
        <div className="space-y-2 pt-2 border-t border-slate-800">
          ...
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-400">Your Earning ({order.customerIsReturning ? "45%" : "40%"})</span>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-200 font-medium">{format(order.deadline, "MMM dd, yyyy 'at' HH:mm")}</p>
          </div>
        </div>
        ...
        <Button onClick={handlePlaceBid} className="w-full bg-primary hover:bg-primary/90" size="lg">
          Place Bid
        </Button>
      </CardContent>
    </Card>
